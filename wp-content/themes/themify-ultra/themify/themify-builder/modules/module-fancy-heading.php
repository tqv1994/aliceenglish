...
class TB_Fancy_Heading_Module extends Themify_Builder_Component_Module {
	...
	<div class="module module-<?php echo $this->slug; ?> {{ data.css_class }}{{ inline }}{{divider}}">
        ...
        <span class="sub-head {{inline}} tf_rel"<# if(!data.sub_heading_link){ #> contenteditable="false" data-name="subheading"<# } #>>
            <# if(!data.sub_heading_link){ #>
				{{{ data.sub_heading }}}<# }
				else{ #>
                <a contenteditable="false" data-name="subheading" href="{{data.sub_heading_link}}">{{{ data.sub_heading }}}</a>
            <# } #>
        </span>
	    </{{ heading_tag }}>
	</div>
