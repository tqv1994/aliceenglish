						...
						if(isImageEdit){
							...
							__callback=function(){
								...
								if(module){
									...
									api.Mixins.Builder.update($(module));
								}
		...
		saveData=function(changeSrc){
			if(activeEl && isChanged){
				...
				if(isOpen){
					...
					const args={val:val,data:v,cid:cid,activeEl:activeEl};
			...
			api.Instances.Builder[api.builderIndex].el.addEventListener(MOUSE['mousedown'], function(e){
				if(e.which===1 || e.touches){
					...
					if(item && !item.closest('.tb_disable_sorting') && (isImageEdit || item.contentEditable===false || item.contentEditable==='false')){
						...
						const _drag= function(e){
							this.removeEventListener('drag', _drag,{passive:true,once:true});
							this.removeEventListener(CLICK['mousedown'], _drag,{passive:true,once:true});
							const activeModule=this.closest('.active_module');
							if(activeModule){
						...
						item.addEventListener('drag', _drag,{passive:true,once:true});
						item.addEventListener(CLICK['mousedown'],_drag,{passive:true,once:true});
					}
				}
			},{passive:true});
						...
						if(activeModule){
							...
							if(isChanged){
								...
								if(!activeEl.hasAttribute('data-no-update')){
									api.Mixins.Builder.update($(activeModule));
								}
							}
						}
					}
