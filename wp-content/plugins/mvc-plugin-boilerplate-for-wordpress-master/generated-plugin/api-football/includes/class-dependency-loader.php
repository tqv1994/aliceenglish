...
if ( ! class_exists( 'Dependency_Loader' ) ) {
	...
	class Dependency_Loader {
		...
		public function load_dependencies( $class ) {
			...
			if ( 'API_Football' !== $parts[0] ) {
				return;
			}

			// Remove 'API_Football' from parts.
			array_shift( $parts );
			...
			$file_path = API_Football::get_plugin_path() . implode( '/', $parts ) . $class_file_name;
			...
			$file_path = API_Football::get_plugin_path() . implode( '/', $parts ) . $trait_file_name;
		...
		protected function load_registries() {
			require_once( API_Football::get_plugin_path() . 'core/registry/trait-base-registry.php' );
			require_once( API_Football::get_plugin_path() . 'core/registry/class-controller.php' );
			require_once( API_Football::get_plugin_path() . 'core/registry/class-model.php' );
		}
		...
		protected function load_core() {
			...
			foreach ( glob( API_Football::get_plugin_path() . 'core/*.php' ) as $path ) {
				require_once $path;
			}
		}
