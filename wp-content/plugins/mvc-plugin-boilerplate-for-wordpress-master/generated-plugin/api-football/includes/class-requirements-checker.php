...
if ( ! class_exists( 'Requirements_Checker' ) ) {
	 ...
	 * @subpackage API_Football/Includes
	 */
	class Requirements_Checker {
		 ...
		 *
		 * @param string $api_football Name of the plugin.
		 * @param string $plugin_slug Slug of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_plugin_active( $api_football, $plugin_slug ) {
			...
			$this->add_error_notice(
				$api_football . ' is a required plugin.',
				$api_football . ' needs to be installed & activated.'
			);
		 ...
		 *
		 * @param string $api_football API Football.
		 * @param string $plugin_slug Plugin Slug.
		 * @param string $min_plugin_version Minimum version required of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_required_plugin_version_active( $api_football, $plugin_slug, $min_plugin_version ) {
			...
			$this->add_error_notice(
				"{$api_football} {$min_plugin_version}+ is required.",
				"{$api_football} {$installed_plugin_version} is installed."
			);
		...
		private function are_required_plugins_dependency_met() {
			...
			$installed_plugins = array_filter(
				...
				function( $required_plugin_data, $required_api_football ) {
					return $this->is_plugin_active( $required_api_football, $required_plugin_data['plugin_slug'] );
				},
			...
			$plugins_installed_with_required_version = array_filter(
				$installed_plugins,
				function( $required_plugin_data, $required_api_football ) {
					return $this->is_required_plugin_version_active( $required_api_football, $required_plugin_data['plugin_slug'], $required_plugin_data['min_plugin_version'] );
				},
				ARRAY_FILTER_USE_BOTH
			);
