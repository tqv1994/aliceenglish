...
class Builder_Contact
{
    ...
    public static function init(){
        ...
        if (is_admin()) {
            add_action('plugins_loaded', array(__CLASS__, 'admin'), 10);
    ...
    public static function admin_enqueue() {
        ...
        wp_enqueue_script('builder-contact-admin-scripts', themify_enque(self::$url . 'assets/admin.js'), array('themify-builder-app-js'), self::$version, true);
        wp_localize_script('builder-contact-admin-scripts', 'tb_contact_l10n', array(
            ...
            'captcha' => Builder_Contact::get_option('recapthca_public_key')?'':sprintf(__('Requires Captcha keys entered at: <a target="_blank" href="%s">reCAPTCHA settings</a>.', 'builder-contact'), admin_url('admin.php?page=builder-contact')),
    ...
    public static function contact_send() {
		...
		$keys = array('public'=>self::get_option('recapthca_public_key'),'private'=>self::get_option('recapthca_private_key'));
		...
		if ( $module_settings['post_type'] === 'enable' ) {
			...
			self::send_via_post_type( $subject, $message . $files_links, $post_author_id );
		}
