...
<div <?php echo self::get_element_attributes(self::sticky_element_props($container_props,$fields_args)); ?>>
		...
		?>
		    <div class="builder-contact-field builder-contact-field-<?php echo $k,($k==='message'?' builder-contact-textarea-field':' builder-contact-text-field')?>">
			    ...
			    <div class="control-input tf_rel">
				    <?php if($k==='message'):?>
					    <textarea name="contact-message" placeholder="<?php echo $placeholder ?>" id="<?php echo $args['module_ID']; ?>-contact-message" class="form-control" required></textarea>
				    <?php else:?>
					...
					<?php elseif(!empty($field['value'])):?>
						...
						<?php elseif( 'select' === $field['type'] ): ?>
							<select id="<?php echo $inputId ?>" name="<?php echo $inputName; ?>" class="form-control tf_scrollbar"<?php echo $required ?>>
								<?php if($required===''):?>
									<option value=""><?php _e('Please select one' , 'builder-contact')?></option>
								<?php endif;?>
	...
	<?php endforeach;?>
	    ...
	    <?php if ( 'yes' === $fields_args['field_captcha_active'] && Builder_Contact::get_option('recapthca_public_key') != '' && Builder_Contact::get_option('recapthca_private_key') != '') : ?>
		<?php $recaptcha_version = Builder_Contact::get_option('recapthca_version','v2'); ?>
		<div class="builder-contact-field builder-contact-field-captcha">
			...
			<?php endif; ?>
		    <div class="control-input tf_rel">
				<div class="themify_captcha_field <?php if ( 'v2' === $recaptcha_version ) :?>g-recaptcha<?php endif; ?>" data-sitekey="<?php echo esc_attr(Builder_Contact::get_option('recapthca_public_key')); ?>" data-ver="<?php esc_attr_e($recaptcha_version); ?>"></div>
		    </div>
		</div>
	    <?php endif; ?>
