...
class AnWPFL_Referee extends CPT_Core {
	 ...
	 * @return array
	 */
	private function get_all_referees() {

		$cache_key = 'FL-REFEREES-LIST-SIMPLE';
	 ...
	 *
	 * @since 0.7.3
	 * @return array $output - Array of all referees
	 */
	public function get_referee_list() {
		...
		if ( null === $output ) {
			...
			$output       = [];
			$all_referees = $this->get_all_referees();

			if ( empty( $all_referees ) ) {
				return [];
			}

			foreach ( $all_referees as $referee ) {
				$output[ $referee->id ] = $referee->pt;
			}
		}
